"""Run the VAE x224 main notebook headlessly in phases."""
...
NOTEBOOK_PATH = Path("experiments/anomaly_detection/vae/x224/main/notebook.ipynb")
...
def resolve_repo_root() -> Path:
    ...
    for candidate in candidates:
        ...
        if (candidate / NOTEBOOK_PATH).exists() and (candidate / "experiments").exists() and (candidate / "scripts").exists():
            return candidate
    raise FileNotFoundError("Could not locate repo root for the VAE x224 main runner.")
...
def execute_phase(notebook_path: Path, *, phase: str, output_dir: str) -> dict[str, Any]:
    ...
    print(
        f"[vae-x224-main] executing {len(phase_cells)} code cells for phase={phase} from {notebook_path}",
        flush=True,
    )
    ...
    for step, cell_index in enumerate(phase_cells, start=1):
        ...
        if cell.get("cell_type") != "code":
            print(
                f"[vae-x224-main] skipping non-code cell {step}/{len(phase_cells)} "
...
""",
        ...
        print(
            f"[vae-x224-main] starting code cell {step}/{len(phase_cells)} "
            f"(phase={phase}, notebook index {cell_index})",
            flush=True,
        )
        ...
        print(
            f"[vae-x224-main] finished code cell {step}/{len(phase_cells)} (phase={phase})",
            flush=True,
        )
...
def main() -> None:
    ...
    print(
        f"[vae-x224-main] repo root: {repo_root}; output_dir={args.output_dir}; phase={args.phase}",
        flush=True,
    )
