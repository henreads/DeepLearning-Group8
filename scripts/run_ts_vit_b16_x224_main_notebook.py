...
DEFAULT_CONFIG = REPO_ROOT / "experiments" / "anomaly_detection" / "teacher_student" / "vit_b16" / "x224" / "main" / "train_config.toml"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "experiments" / "anomaly_detection" / "teacher_student" / "vit_b16" / "x224" / "main" / "artifacts" / "ts_vit_b16_x224"
...
def _threshold_quantile(config_path: Path) -> float:
    text = config_path.read_text(encoding="utf-8")
    for raw_line in text.splitlines():
        line = raw_line.strip()
        if line.startswith("threshold_quantile"):
...
def run_eval(config_path: Path, output_dir: Path) -> None:
    ...
    _run(
        [
            ...
            "ts_distillation",
            "--threshold-quantile",
            str(_threshold_quantile(config_path)),
            "--output-dir",
            str(results_dir),
        ]
    )
...
def run_sweep(config_path: Path, output_dir: Path) -> None:
    ...
    threshold_quantile = _threshold_quantile(config_path)
    ...
    score_sweep_df = pd.DataFrame(rows).sort_values(["f1", "auroc", "auprc"], ascending=[False, False, False]).reset_index(drop=True)
