"""Run the teacher-student WideResNet50-2 multilayer x224 notebook headlessly in phases."""
...
def resolve_repo_root() -> Path:
    ...
    for candidate in candidates:
        ...
        if (candidate / NOTEBOOK_PATH).exists() and (candidate / "experiments").exists() and (candidate / "scripts").exists():
            return candidate
    raise FileNotFoundError("Could not locate repo root for the TS WideResNet50-2 multilayer x224 runner.")
...
def execute_phase(notebook_path: Path, *, phase: str, output_dir: str, num_workers: int) -> dict[str, Any]:
    ...
    print(
        f"[ts-wrn50-x224-multilayer] executing {len(phase_cells)} code cells for phase={phase} from {notebook_path}",
        flush=True,
    )
...
""",
        ...
        print(
            f"[ts-wrn50-x224-multilayer] starting code cell {step}/{len(phase_cells)} "
        ...
        print(
            f"[ts-wrn50-x224-multilayer] finished code cell {step}/{len(phase_cells)} (phase={phase})",
            flush=True,
        )
...
def main() -> None:
    ...
    print(
        f"[ts-wrn50-x224-multilayer] repo root: {repo_root}; output_dir={args.output_dir}; "
