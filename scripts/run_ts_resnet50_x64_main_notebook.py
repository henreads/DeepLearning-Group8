"""Run the teacher-student ResNet50 x64 main notebook headlessly in phases."""
...
NOTEBOOK_PATH = Path("experiments/anomaly_detection/teacher_student/resnet50/x64/main/notebook.ipynb")
...
def resolve_repo_root() -> Path:
    ...
    for candidate in candidates:
        ...
        if (candidate / NOTEBOOK_PATH).exists() and (candidate / "experiments").exists() and (candidate / "scripts").exists():
            return candidate
    raise FileNotFoundError("Could not locate repo root for the TS ResNet50 x64 main runner.")
...
def execute_phase(notebook_path: Path, *, phase: str, output_dir: str) -> dict[str, Any]:
    ...
    print(
        f"[ts-resnet50-x64-main] executing {len(phase_cells)} code cells for phase={phase} from {notebook_path}",
        flush=True,
    )
...
""",
        ...
        print(
            f"[ts-resnet50-x64-main] starting code cell {step}/{len(phase_cells)} "
        ...
        print(
            f"[ts-resnet50-x64-main] finished code cell {step}/{len(phase_cells)} (phase={phase})",
            flush=True,
        )
...
def main() -> None:
    ...
    print(
        f"[ts-resnet50-x64-main] repo root: {repo_root}; output_dir={args.output_dir}; phase={args.phase}",
        flush=True,
    )
