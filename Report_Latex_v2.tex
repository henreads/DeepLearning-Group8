...
from its original resolution to the backbone's native input size in a single nearest-neighbour step: $224\!\times\!224$
for WideResNet50-2 and ViT-B/16, and $240\!\times\!240$ for EfficientNet-B1. Each backbone is then compared against
...
The autoencoder family achieved a best AUPRC of $0.596$ ($\mathrm{F1}=0.510$) at native $224\!\times\!224$ resolution, establishing the reconstruction-based ceiling. Across all variants, the pattern is consistent: broad defects are well-detected, small localised defects are missed. Key findings:
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Deep SVDD:} Uses the same encoder architecture but trains a one-class geometric objective, minimising the volume of a latent hypersphere enclosing all normal embeddings. Wafers outside the hypersphere are flagged as anomalous. $\mathrm{F1}=0.360$, which is stronger than every VAE variant (best VAE: $\mathrm{F1}=0.343$) but still well below the tuned reconstruction baseline, indicating that a single hypersphere cannot replace reconstruction-based scoring on this domain.
\end{itemize}
...
Training runs for up to 50 epochs with early stopping; the best validation loss checkpoint is saved at epoch 38. The choice of reduction rule for aggregating per-pixel errors into a single wafer score substantially affects performance.
...
\textbf{Architectural modifications.} BatchNormalization, dropout (0--0.20), and residual skip connections are tested, but none address the root problem: the scoring function. Rescoring the BatchNorm checkpoint with \texttt{max\_abs} (worst pixel error instead of mean) raises F1 from 0.431 to 0.502 without any retraining. Score choice is as impactful as architecture.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{images/autoencoder/score_ablation_summary.png}
...
This ceiling (AUPRC=0.596, F1=0.510) becomes the benchmark for Phase 2 and Phase 3: any improvement beyond reconstruction must fundamentally change how anomaly signals are aggregated (from global, full-image average, to local, patch-level scores).
...
The MAE variants use block 6 features, a 128-d projection head, a 600k-token coreset memory bank, and a 95th-percentile threshold. The frozen baseline uses a 400k-token memory bank; all other settings are identical, so the only controlled variable is the backbone weights.
...
\textbf{Spatial feature visualization.}
Figure~\ref{fig:dinov2_viz} shows three complementary spatial heatmaps for edge-loc and edge-ring defect classes under the block-6 DINOv2 PatchCore model. Both classes produce spatially compact heatmaps aligned with the defect region, confirming that block-6 features retain sufficient local structure to localise edge-type defects reliably.

\begin{itemize}
    \item \textbf{PatchCore Heatmap.} Per-patch nearest-neighbour distance to the normal memory bank. Brighter regions indicate patches most dissimilar from normal wafers.
    \item \textbf{Occlusion Map.} Drop in anomaly score when each 14$\times$14 patch is replaced with a background token. Directly measures each patch's contribution to the detection decision.
    \item \textbf{Sensitivity Map.} L2 norm of the gradient of the anomaly score with respect to each patch's block-6 features. Highlights which patches the score is most sensitive to.
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{images/vit/main/viz_Edge_Loc.png}
\includegraphics[width=0.9\linewidth]{images/vit/main/viz_Edge_Ring.png}
\caption{Spatial heatmaps for edge-loc and edge-ring defect classes under DINOv2 ViT-B/14 PatchCore (block~6). Each row shows one wafer image. Columns from left to right: raw wafer map, PatchCore distance heatmap, occlusion map, and sensitivity map. Misses (model predicted normal) are labelled Normal (MISS).}
\label{fig:dinov2_viz}
\end{figure}
...
\textbf{Experimental setup.} A linear probe was trained on top of frozen ViT-B/16 [CLS] embeddings to classify wafers as normal or defective. Training used 40,000 normal wafers and a varying fraction of labeled defect samples from six seen classes (Center, Edge-Ring, Edge-Loc, Donut, Near-full, Random). Two classes, Scratch and Loc, were completely withheld from training and validation. The test set matches the PatchCore benchmark exactly: 5,000 normal wafers plus 250 defect wafers stratified across all eight classes. The 95th-percentile validation-normal threshold was applied identically to all models, making AUROC and F1 directly comparable to the zero-defect PatchCore ViT-B/16 result (AUROC = 0.956, F1 = 0.595). Six labeled-defect fractions were swept, yielding 20, 51, 102, 203, 508, and 1,016 training defect samples.
