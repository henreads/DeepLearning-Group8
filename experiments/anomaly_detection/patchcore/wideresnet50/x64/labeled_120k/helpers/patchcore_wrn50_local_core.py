...
def resolve_bundle_root(start: str | Path | None = None) -> Path:
    ...
    for candidate in [start_path, *start_path.parents]:
        if (candidate / "helpers" / "patchcore_wrn50_local_core.py").exists() and (candidate / "README.md").exists():
            return candidate
        if (
            (
                candidate
                ...
                / "patchcore_wrn50"
                / "helpers"
                / "patchcore_wrn50_local_core.py"
            ).exists()
            and (candidate / "configs").exists()
        ):
            return candidate
    raise FileNotFoundError("Could not locate the local WRN PatchCore bundle root.")
