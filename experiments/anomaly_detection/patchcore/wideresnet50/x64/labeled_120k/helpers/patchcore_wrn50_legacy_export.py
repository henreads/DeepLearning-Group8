...
def resolve_bundle_root(start: str | Path | None = None) -> Path:
    ...
    for candidate in [start_path, *start_path.parents]:
        if (candidate / "helpers" / "patchcore_wrn50_legacy_export.py").exists() and (candidate / "README.md").exists():
            return candidate
        if (
            (
                candidate
                ...
                / "patchcore_wrn50"
                / "helpers"
                / "patchcore_wrn50_legacy_export.py"
            ).exists()
            and (candidate / "configs").exists()
        ):
            return candidate
    raise FileNotFoundError("Could not locate the legacy local export bundle root.")
