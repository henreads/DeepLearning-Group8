...
APP_NAME = "wafer-defect-patchcore-dinov2-vit-b14-x224-main"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x224-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-dinov2-vit-b14-x224-main-artifacts"
...
LOCAL_ARTIFACT_DIR = LOCAL_X224_DIR / "block9" / "artifacts"
LOCAL_SWEEP_ARTIFACT_DIR = LOCAL_X224_DIR
...
REMOTE_ARTIFACT_ROOT = "/root/artifacts/patchcore_dinov2_vit_b14_x224"
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_dinov2_vit_b14_x224_main_notebook.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/dinov2_vit_b14/x224",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/dinov2_vit_b14/x224",
        copy=True,
        ignore=[
            "artifacts",
            "artifacts/**",
...
def _remote_subdir_candidates_for_block(block: int) -> list[str]:
    if block == 9:
        return ["/block9/artifacts"]
    return [f"/block{block}/artifacts", f"/block_{block}/artifacts"]


def _resolve_existing_remote_subdir(remote_subdir: str, vit_feature_block: int | None = None) -> str:
    candidates = [remote_subdir]
    if vit_feature_block is not None and remote_subdir != "/":
        candidates = _remote_subdir_candidates_for_block(vit_feature_block)

    for candidate in candidates:
        try:
            _run_modal_cli(["volume", "ls", ARTIFACT_VOLUME_NAME, candidate, "--json"], capture_output=True)
            return candidate
    ...
    raise FileNotFoundError(f"Remote artifact directory not found for block {vit_feature_block}: tried {candidates}")
...
def _artifact_dir_for_block(block: int) -> str:
    if block == 9:
        return f"{REMOTE_ARTIFACT_ROOT}/block9/artifacts"
    return f"{REMOTE_ARTIFACT_ROOT}/block{block}/artifacts"


def _artifact_subdir_for_block(block: int) -> str:
    if block == 9:
        return "/block9/artifacts"
    return f"/block{block}/artifacts"
...
def run_patchcore_remote(
    num_workers: int = 8,
    batch_size: int = 128,
    vit_feature_block: int = 9,
    force_rerun: bool = False,
) -> dict[str, Any]:
    metadata_path = _prepare_processed_dataset()
    remote_output_dir = _artifact_dir_for_block(vit_feature_block)
    command = [
        "python",
        "-u",
        REMOTE_RUNNER,
        ...
        str(batch_size),
        "--vit-feature-block",
        str(vit_feature_block),
    ]
    if force_rerun:
        command.append("--force-rerun")
    print(f"[patchcore-dinov2-vit-b14-x224-main] launching runner: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    artifact_volume.commit()
    manifest_path = Path(remote_output_dir) / "run_manifest.json"
    if manifest_path.exists():
        return json.loads(manifest_path.read_text(encoding="utf-8"))
    return {"output_dir": remote_output_dir, "vit_feature_block": vit_feature_block}
...
def main(
    ...
    result = run_patchcore_remote.remote(
    ...
    if sync_back:
        _download_artifacts(str(LOCAL_X224_DIR), _artifact_subdir_for_block(vit_feature_block))
...
def sweep_blocks(
    ...
    for block in (6, 9, 11):
        result = run_patchcore_remote.remote(
            num_workers=num_workers,
            batch_size=batch_size,
            vit_feature_block=block,
            force_rerun=force_rerun,
        )
        results.append(result)
        if sync_back:
            _download_artifacts(str(LOCAL_X224_DIR), _artifact_subdir_for_block(block))
...
def download_artifacts(
    local_artifact_dir: str = str(LOCAL_X224_DIR),
    vit_feature_block: int = -1,
) -> None:
    remote_subdir = "/" if vit_feature_block < 0 else _resolve_existing_remote_subdir(
        _artifact_subdir_for_block(vit_feature_block),
        vit_feature_block,
    )
    _download_artifacts(local_artifact_dir, remote_subdir)
