...
APP_NAME = "wafer-defect-backbone-resnet18-x64-holdout"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x64-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-backbone-resnet18-x64-holdout-artifacts"
...
REMOTE_REFERENCE_ARTIFACT_DIR = "/root/reference_artifacts/backbone_resnet18_x64"
REMOTE_RAW_DIR = f"{REMOTE_PROJECT_ROOT}/data/raw"
REMOTE_PROCESSED_DIR = f"{REMOTE_PROJECT_ROOT}/data/processed/x64/wm811k"
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/backbone_embedding/resnet18/x64/baseline/artifacts"
)
...
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_resnet18_backbone_x64_holdout.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/backbone_embedding/resnet18/x64/baseline",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/backbone_embedding/resnet18/x64/baseline",
        copy=True,
        ignore=["artifacts", "artifacts/**"],
    )
    .add_local_dir(
        "experiments/anomaly_detection/backbone_embedding/resnet18/x64/baseline/artifacts",
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(f"[backbone-resnet18-x64-holdout] reusing cached processed dataset: {metadata_path}", flush=True)
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[backbone-resnet18-x64-holdout] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/backbone_resnet18_x64_holdout_prepare.toml")
    ...
    print(f"[backbone-resnet18-x64-holdout] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[backbone-resnet18-x64-holdout] processed dataset volume committed", flush=True)
...
def _prepare_holdout_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(f"[backbone-resnet18-x64-holdout] reusing cached holdout metadata: {metadata_path}", flush=True)
        return
    ...
    print(f"[backbone-resnet18-x64-holdout] building holdout metadata: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[backbone-resnet18-x64-holdout] holdout metadata volume committed", flush=True)
...
def run_holdout_remote() -> dict[str, Any]:
    ...
    command = [
        ...
        "data/processed/x64/wm811k/metadata_50k_5pct_holdout70k_3p5k.csv",
        "--input-artifact-dir",
        f"{REMOTE_REFERENCE_ARTIFACT_DIR}/artifacts/resnet18_embedding_baseline",
        "--output-dir",
        "experiments/anomaly_detection/backbone_embedding/resnet18/x64/baseline/artifacts/resnet18_embedding_baseline/holdout70k_3p5k",
    ]
    print(f"[backbone-resnet18-x64-holdout] launching holdout runner: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    artifact_volume.commit()
    print("[backbone-resnet18-x64-holdout] artifact volume committed", flush=True)
    manifest_path = Path(REMOTE_ARTIFACT_DIR) / "resnet18_embedding_baseline" / "holdout70k_3p5k" / "run_manifest.json"
...
def main(sync_back: bool = True) -> None:
    result = run_holdout_remote.remote()
    print(json.dumps(result, indent=2))
    if sync_back:
        _download_artifacts(str(LOCAL_ARTIFACT_DIR), "/resnet18_embedding_baseline/holdout70k_3p5k")
...
def download_artifacts(local_artifact_dir: str = str(LOCAL_ARTIFACT_DIR)) -> None:
    _download_artifacts(local_artifact_dir, "/resnet18_embedding_baseline/holdout70k_3p5k")
