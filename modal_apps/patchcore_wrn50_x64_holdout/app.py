...
APP_NAME = "wafer-defect-patchcore-wrn50-x64-holdout"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x64-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-wrn50-x64-holdout-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "patchcore" / "wideresnet50" / "x64" / "main" / "artifacts"
)

REMOTE_PROJECT_ROOT = "/root/project"
REMOTE_REFERENCE_ARTIFACT_DIR = "/root/reference_artifacts/patchcore_wrn50_x64"
REMOTE_RAW_DIR = f"{REMOTE_PROJECT_ROOT}/data/raw"
REMOTE_PROCESSED_DIR = f"{REMOTE_PROJECT_ROOT}/data/processed/x64/wm811k"
REMOTE_ARTIFACT_DIR = f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/wideresnet50/x64/main/artifacts"
...
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_wrn50_x64_holdout.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/wideresnet50/x64/main",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/wideresnet50/x64/main",
        copy=True,
        ignore=["artifacts", "artifacts/**"],
    )
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/wideresnet50/x64/main/artifacts",
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(f"[patchcore-wrn50-x64-holdout] reusing cached processed dataset: {metadata_path}", flush=True)
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[patchcore-wrn50-x64-holdout] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/patchcore_wrn50_x64_holdout_prepare.toml")
    ...
    print(f"[patchcore-wrn50-x64-holdout] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[patchcore-wrn50-x64-holdout] processed dataset volume committed", flush=True)
...
def _prepare_holdout_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(f"[patchcore-wrn50-x64-holdout] reusing cached holdout metadata: {metadata_path}", flush=True)
        return
    ...
    print(f"[patchcore-wrn50-x64-holdout] building holdout metadata: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[patchcore-wrn50-x64-holdout] holdout metadata volume committed", flush=True)
...
def run_holdout_remote(variant_name: str = "", num_workers: int = 4) -> dict[str, Any]:
    ...
    command = [
        ...
        "data/processed/x64/wm811k/metadata_50k_5pct_holdout70k_3p5k.csv",
        "--input-artifact-dir",
        f"{REMOTE_REFERENCE_ARTIFACT_DIR}/artifacts/patchcore_wideresnet50_multilayer",
        "--output-dir",
        "experiments/anomaly_detection/patchcore/wideresnet50/x64/main/artifacts/patchcore_wideresnet50_multilayer/holdout70k_3p5k",
        "--num-workers",
        str(num_workers),
    ]
    if variant_name:
        command.extend(["--variant-name", variant_name])
    print(f"[patchcore-wrn50-x64-holdout] launching holdout runner: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    artifact_volume.commit()
    print("[patchcore-wrn50-x64-holdout] artifact volume committed", flush=True)
    manifest_path = Path(REMOTE_ARTIFACT_DIR) / "patchcore_wideresnet50_multilayer" / "holdout70k_3p5k" / "run_manifest.json"
...
def main(variant_name: str = "", num_workers: int = 4, sync_back: bool = True) -> None:
    result = run_holdout_remote.remote(variant_name=variant_name, num_workers=num_workers)
    print(json.dumps(result, indent=2))
    if sync_back:
        _download_artifacts(str(LOCAL_ARTIFACT_DIR), "/patchcore_wideresnet50_multilayer/holdout70k_3p5k")
...
def download_artifacts(local_artifact_dir: str = str(LOCAL_ARTIFACT_DIR)) -> None:
    _download_artifacts(local_artifact_dir, "/patchcore_wideresnet50_multilayer/holdout70k_3p5k")
