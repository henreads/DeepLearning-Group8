...
APP_NAME = "wafer-defect-patchcore-vit-b16-x64-main"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-vit-b16-x64-main-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "patchcore" / "vit_b16" / "x64" / "main" / "artifacts"
)
REMOTE_PROJECT_ROOT = "/root/project"
REMOTE_RAW_DIR = f"{REMOTE_PROJECT_ROOT}/data/raw"
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/vit_b16/x64/main/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_vit_b16_x64_main_notebook.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/vit_b16/x64/main",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/vit_b16/x64/main",
        copy=True,
...
def run_patchcore_remote(num_workers: int = 0, batch_size: int = 256, force_rerun: bool = False) -> dict[str, Any]:
    command = [
        "python",
        "-u",
        REMOTE_RUNNER,
        ...
        "--output-dir",
        "experiments/anomaly_detection/patchcore/vit_b16/x64/main/artifacts",
        "--num-workers",
        str(num_workers),
        "--batch-size",
        str(batch_size),
    ...
    print(f"[patchcore-vit-b16-x64-main] launching runner: {' '.join(command)}", flush=True)
...
def main(num_workers: int = 0, batch_size: int = 256, force_rerun: bool = False, sync_back: bool = True) -> None:
