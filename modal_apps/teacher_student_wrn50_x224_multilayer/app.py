...
APP_NAME = "wafer-defect-ts-wrn50-x224-multilayer"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x224-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-ts-wrn50-x224-multilayer-artifacts"
...
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/wideresnet50_2/x224/multilayer_self_contained/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_ts_wrn50_x224_multilayer_notebook.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/teacher_student/wideresnet50_2/x224/multilayer_self_contained",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/wideresnet50_2/x224/multilayer_self_contained",
        copy=True,
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(
            f"[ts-wrn50-x224-multilayer] reusing cached processed dataset: {metadata_path}",
            flush=True,
        )
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[ts-wrn50-x224-multilayer] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/ts_wrn50_x224_prepare_wm811k.toml")
    ...
    print(f"[ts-wrn50-x224-multilayer] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[ts-wrn50-x224-multilayer] processed dataset volume committed", flush=True)
...
def run_ts_remote(num_workers: int = 8) -> dict[str, Any]:
    ...
    for phase in ["train", "eval", "sweep"]:
        command = [
            ...
            REMOTE_RUNNER,
            "--output-dir",
            "experiments/anomaly_detection/teacher_student/wideresnet50_2/x224/multilayer_self_contained/artifacts/ts_wideresnet50_multilayer",
            "--phase",
            phase,
            "--num-workers",
            str(num_workers),
        ]
        print(f"[ts-wrn50-x224-multilayer] launching phase={phase}: {' '.join(command)}", flush=True)
        subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
        artifact_volume.commit()
        print(f"[ts-wrn50-x224-multilayer] artifact volume committed after phase={phase}", flush=True)
