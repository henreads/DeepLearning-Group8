...
APP_NAME = "wafer-defect-patchcore-effb0-x224-main"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x224-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-effb0-x224-main-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "patchcore" / "efficientnet_b0" / "x224" / "main" / "artifacts"
)
...
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/efficientnet_b0/x224/main/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_effb0_x224_main_notebook.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/efficientnet_b0/x224/main",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/efficientnet_b0/x224/main",
        copy=True,
        ignore=["artifacts", "artifacts/**"],
    )
    .add_local_dir(
        "experiments/archive/anomaly_50k_duplicates",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/archive/anomaly_50k_duplicates",
        copy=True,
    )
)
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(
            f"[patchcore-effb0-x224-main] reusing cached processed dataset: {metadata_path}",
            flush=True,
        )
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[patchcore-effb0-x224-main] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/patchcore_effb0_x224_prepare_wm811k.toml")
    ...
    print(f"[patchcore-effb0-x224-main] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[patchcore-effb0-x224-main] processed dataset volume committed", flush=True)
...
def run_patchcore_remote(num_workers: int = 4, batch_size: int = 128) -> dict[str, Any]:
    ...
    command = [
        ...
        REMOTE_RUNNER,
        "--metadata-path",
        f"{REMOTE_PROCESSED_DIR}/metadata_50k_5pct.csv",
        "--output-dir",
    ...
    print(f"[patchcore-effb0-x224-main] launching notebook runner: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    artifact_volume.commit()
    print("[patchcore-effb0-x224-main] artifact volume committed", flush=True)
    manifest_path = Path(REMOTE_ARTIFACT_DIR) / "patchcore_efficientnet_b0_5pct" / "run_manifest.json"
...
def main(num_workers: int = 4, batch_size: int = 128, sync_back: bool = True) -> None:
    ...
    if sync_back:
        _download_artifacts(str(LOCAL_ARTIFACT_DIR), "/patchcore_efficientnet_b0_5pct")
...
def download_artifacts(local_artifact_dir: str = str(LOCAL_ARTIFACT_DIR)) -> None:
    _download_artifacts(local_artifact_dir, "/patchcore_efficientnet_b0_5pct")
