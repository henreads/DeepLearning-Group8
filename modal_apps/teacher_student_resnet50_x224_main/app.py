...
APP_NAME = "wafer-defect-ts-resnet50-x224-main"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x224-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-ts-resnet50-x224-main-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "teacher_student" / "resnet50" / "x224" / "main" / "artifacts"
)
...
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/resnet50/x224/main/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_ts_resnet50_x224_main_notebook.py"
...
image = (
    ...
    .add_local_file(
        ...
        remote_path=f"{REMOTE_PROJECT_ROOT}/scripts/run_ts_resnet50_x224_main_notebook.py",
        copy=True,
    )
    .add_local_dir(
        "experiments/anomaly_detection/teacher_student/resnet50/x224/main",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/resnet50/x224/main",
        copy=True,
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(
            f"[ts-resnet50-x224-main] reusing cached processed dataset: {metadata_path}",
            flush=True,
        )
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[ts-resnet50-x224-main] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/ts_resnet50_x224_prepare_wm811k.toml")
    ...
    print(f"[ts-resnet50-x224-main] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[ts-resnet50-x224-main] processed dataset volume committed", flush=True)
...
def run_ts_remote(fresh_train: bool = False, quick: bool = True, epochs: int | None = None, run_sweep: bool = False) -> dict[str, Any]:
    ...
    effective_config = Path("/tmp/ts_resnet50_x224_effective.toml")
    config_text = Path(f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/resnet50/x224/main/train_config.toml").read_text(encoding="utf-8")
    effective_epochs = int(epochs if epochs is not None else (QUICK_SWEEP_EPOCHS if quick else FULL_SWEEP_EPOCHS))
    config_text = config_text.replace("batch_size = 256", "batch_size = 256")
    ...
    for phase in phases:
        command = [
            ...
            "--output-dir",
            "experiments/anomaly_detection/teacher_student/resnet50/x224/main/artifacts/ts_resnet50_x224",
            "--phase",
            phase,
        ]
        ...
        print(f"[ts-resnet50-x224-main] artifact volume committed after phase={phase}", flush=True)
        manifest_path = Path(REMOTE_ARTIFACT_DIR) / "ts_resnet50_x224" / f"{phase}_phase_manifest.json"
        if manifest_path.exists():
            manifests[phase] = json.loads(manifest_path.read_text(encoding="utf-8"))
    final_manifest = Path(REMOTE_ARTIFACT_DIR) / "ts_resnet50_x224" / "run_manifest.json"
...
def download_artifacts(local_artifact_dir: str = str(LOCAL_ARTIFACT_DIR)) -> None:
    _download_artifacts(local_artifact_dir, "/ts_resnet50_x224")
