...
APP_NAME = "wafer-defect-patchcore-wrn50-x64"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-wrn50-x64-artifacts"
...
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/wideresnet50/x64/main/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_wrn50_x64_notebook.py"
...
def run_patchcore_remote(num_workers: int = 4) -> dict[str, Any]:
    ...
    print(f"[patchcore-wrn50-x64] launching notebook runner: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    artifact_volume.commit()
    print("[patchcore-wrn50-x64] artifact volume committed", flush=True)

    manifest_path = Path(REMOTE_ARTIFACT_DIR) / "patchcore_wideresnet50_multilayer" / "run_manifest.json"
