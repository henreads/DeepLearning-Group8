...
APP_NAME = "wafer-defect-ts-resnet50-x64-main"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
PROCESSED_VOLUME_NAME = "wafer-defect-wm811k-x64-processed"
ARTIFACT_VOLUME_NAME = "wafer-defect-ts-resnet50-x64-main-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "teacher_student" / "resnet50" / "x64" / "main" / "artifacts"
)
...
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/resnet50/x64/main/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_ts_resnet50_x64_main_notebook.py"
...
image = (
    ...
    .add_local_dir(
        ...
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/teacher_student/resnet50/x64/main",
        copy=True,
...
def _prepare_processed_dataset() -> None:
    ...
    if _cached_dataset_is_valid(metadata_path, arrays_dir):
        print(
            f"[ts-resnet50-x64-main] reusing cached processed dataset: {metadata_path}",
            flush=True,
        )
        return
    if metadata_path.exists() or arrays_dir.exists():
        print("[ts-resnet50-x64-main] cached processed dataset is stale; rebuilding", flush=True)
        _clear_processed_cache(metadata_path, arrays_dir)

    config_path = Path("/tmp/ts_resnet50_x64_prepare_wm811k.toml")
    ...
    print(f"[ts-resnet50-x64-main] preparing processed dataset: {' '.join(command)}", flush=True)
    subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
    processed_volume.commit()
    print("[ts-resnet50-x64-main] processed dataset volume committed", flush=True)
...
def run_ts_remote() -> dict[str, Any]:
    ...
    for phase in ["train", "eval", "sweep"]:
        command = [
            ...
            REMOTE_RUNNER,
            "--output-dir",
            "experiments/anomaly_detection/teacher_student/resnet50/x64/main/artifacts/ts_resnet50",
            "--phase",
            phase,
        ]
        print(f"[ts-resnet50-x64-main] launching phase={phase}: {' '.join(command)}", flush=True)
        subprocess.run(command, check=True, cwd=REMOTE_PROJECT_ROOT)
        artifact_volume.commit()
        print(f"[ts-resnet50-x64-main] artifact volume committed after phase={phase}", flush=True)
        manifest_path = Path(REMOTE_ARTIFACT_DIR) / "ts_resnet50" / f"{phase}_phase_manifest.json"
        if manifest_path.exists():
            manifests[phase] = json.loads(manifest_path.read_text(encoding="utf-8"))
    final_manifest = Path(REMOTE_ARTIFACT_DIR) / "ts_resnet50" / "run_manifest.json"
...
def main(sync_back: bool = True) -> None:
    ...
    if sync_back:
        _download_artifacts(str(LOCAL_ARTIFACT_DIR), "/ts_resnet50")
...
def download_artifacts(local_artifact_dir: str = str(LOCAL_ARTIFACT_DIR)) -> None:
    _download_artifacts(local_artifact_dir, "/ts_resnet50")
