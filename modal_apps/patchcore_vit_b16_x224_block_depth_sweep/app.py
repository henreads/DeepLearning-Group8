...
APP_NAME = "wafer-defect-patchcore-vit-b16-x224-block-depth-sweep"
RAW_VOLUME_NAME = "wafer-defect-lswmd-raw"
ARTIFACT_VOLUME_NAME = "wafer-defect-patchcore-vit-b16-x224-block-depth-sweep-artifacts"
...
LOCAL_ARTIFACT_DIR = (
    LOCAL_REPO_ROOT / "experiments" / "anomaly_detection" / "patchcore" / "vit_b16" / "x224" / "block_depth_sweep" / "artifacts"
)
REMOTE_PROJECT_ROOT = "/root/project"
REMOTE_RAW_DIR = f"{REMOTE_PROJECT_ROOT}/data/raw"
REMOTE_ARTIFACT_DIR = (
    f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/vit_b16/x224/block_depth_sweep/artifacts"
)
REMOTE_RUNNER = f"{REMOTE_PROJECT_ROOT}/scripts/run_patchcore_vit_b16_x224_block_depth_sweep_notebook.py"
...
image = (
    ...
    .add_local_dir(
        "experiments/anomaly_detection/patchcore/vit_b16/x224/block_depth_sweep",
        remote_path=f"{REMOTE_PROJECT_ROOT}/experiments/anomaly_detection/patchcore/vit_b16/x224/block_depth_sweep",
        copy=True,
...
def run_patchcore_remote(num_workers: int = 0, batch_size: int = 128, force_rerun: bool = False) -> dict[str, Any]:
    command = [
        "python",
        "-u",
        REMOTE_RUNNER,
        ...
        "--output-dir",
        "experiments/anomaly_detection/patchcore/vit_b16/x224/block_depth_sweep/artifacts",
        "--num-workers",
        str(num_workers),
        "--batch-size",
        str(batch_size),
    ...
    print(f"[patchcore-vit-b16-x224-block-depth-sweep] launching runner: {' '.join(command)}", flush=True)
...
def main(num_workers: int = 0, batch_size: int = 128, force_rerun: bool = False, sync_back: bool = True) -> None:
    result = run_patchcore_remote.remote(num_workers=num_workers, batch_size=batch_size, force_rerun=force_rerun)
